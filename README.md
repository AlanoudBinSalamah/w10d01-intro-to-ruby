...
### Preparation
- For mac:
  - type in terminal: `\curl -sSL https://get.rvm.io | bash -s stable --rails`
  - Make sure `[[ -s "$HOME/.rvm/scripts/rvm" ]] && source "$HOME/.rvm/scripts/rvm" # Load RVM into a shell session *as a function*` is in your bash profile and restart your terminal.
- For windows [follow these directions](https://github.com/rvm/ubuntu_rvm)
...
> In ruby the convention is to use `?` (predicate) for methods that return true or false . (in JS we use verbs like `is` or `did`)
